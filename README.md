...
How It Works
Baseline mode (--baseline) — the target file is read in 4KB chunks and hashed using SHA-256 (via OpenSSL's EVP API). The resulting hash is stored in a local SQLite database (fim.db), keyed by filepath.

Watch mode (--watch) — the program registers the target file with the Linux kernel's inotify subsystem. It then blocks on read(), consuming zero CPU until the kernel reports a modification event.

Detection — when a modification is detected, the file is rehashed and compared against the stored baseline using strcmp(). A mismatch triggers a tamper alert.
## Project Structure
...
libsqlite3-dev
## Demo
![Baseline and watchdemo](/screenshots/BaselineDemo.png)
